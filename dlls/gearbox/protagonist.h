...
//=========================================================
// CProtagonistMonster - all the extra data for monsters that
// form squads and are player allies
//=========================================================

#define	SF_PROTAGONIST_LEADER	32


#define bits_NO_SLOT		0

// HUMAN GRUNT SLOTS
#define bits_SLOT_PROTAGONIST_ENGAGE1	( 1 << 0 )
#define bits_SLOT_PROTAGONIST_ENGAGE2	( 1 << 1 )
#define bits_SLOTS_PROTAGONIST_ENGAGE	( bits_SLOT_PROTAGONIST_ENGAGE1 | bits_SLOT_PROTAGONIST_ENGAGE2 )

#define bits_SLOT_PROTAGONIST_GRENADE1	( 1 << 2 ) 
#define bits_SLOT_PROTAGONIST_GRENADE2	( 1 << 3 ) 
#define bits_SLOTS_PROTAGONIST_GRENADE	( bits_SLOT_PROTAGONIST_GRENADE1 | bits_SLOT_PROTAGONIST_GRENADE2 )
...
#define	MAXPROT_SQUAD_MEMBERS	2
...
enum
{
	...
	TASK_PRT_RESPOND,		// say my response
	TASK_PRT_SPEAK,			// question or remark
	TASK_PRT_HELLO,			// Try to say hello to player
	TASK_PRT_HEADRESET,		// reset head position
	TASK_PRT_STOPSHOOTING,	// tell player to stop shooting friend
	TASK_PRT_STARE,			// let the player know I know he's staring at me.
	TASK_PRT_LOOK_AT_CLIENT,// faces player if not moving and not talking and in idle.
	TASK_PRT_CLIENT_STARE,	// same as look at client, but says something if the player stares.
	TASK_PRT_EYECONTACT,	// maintain eyecontact with person who I'm talking to
	TASK_PRT_IDEALYAW,		// set ideal yaw to face who I'm talking to
	TASK_FACE_PLAYER,		// Face the player

	LAST_TALKMONSTER_TASK,			// MUST be last
};

class CProtagonistMonster : public CBaseMonster
{
public:
	void			TalkInit( void );				
	...
	virtual void	SetAnswerQuestion( CProtagonistMonster *pSpeaker );
	...
	static char *m_szPFriends[PRT_CFRIENDS];		// array of friend names
	...
	EHANDLE	m_hSquadMember[MAXPROT_SQUAD_MEMBERS-1];	// valid only for leader
	int		m_afSquadSlots;
	...
	CProtagonistMonster *MySquadLeader( ) 
	{ 
		CProtagonistMonster *pSquadLeader = (CProtagonistMonster *)((CBaseEntity *)m_hSquadLeader); 
		if (pSquadLeader != NULL)
			return pSquadLeader;
		return this;
	}
	CProtagonistMonster *MySquadMember( int i ) 
	{ 
		if (i >= MAXPROT_SQUAD_MEMBERS-1)
			return this;
		else
			return (CProtagonistMonster *)((CBaseEntity *)m_hSquadMember[i]); 
	}
	...
	int	SquadCount( void );
	void SquadRemove( CProtagonistMonster *pRemove );
	void SquadUnlink( void );
	BOOL SquadAdd( CProtagonistMonster *pAdd );
	...
	virtual CProtagonistMonster *MyProtagonistMonsterPointer( void ) { return this; }
